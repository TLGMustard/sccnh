...
test('rejects non-JSON requests before application code reads them', () => {
  assert.deepEqual(parseJsonRequest('text/plain', '{"action":"mine"}'), { ok: false, status: 415 });
});

test('rejects malformed or oversized JSON requests', () => {
...
test('rejects an oversized declared body before reading it', () => {
...
test('limits repeated access attempts and clears the window later', () => {
...
test('marks private responses as non-cacheable and non-referring', () => {
