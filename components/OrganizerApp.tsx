...
function VolunteerRoster({ volunteers, mutate }: { volunteers: AdminVolunteer[]; mutate: (action: AdminAction) => Promise<boolean> }) {
  ...
  async function toggle(volunteer: AdminVolunteer) {
    const key = `${volunteer.id}:general`;
    setPending(key);
...
function CheckIn({ data, mutate }: { data: AdminSnapshot; mutate: (action: AdminAction) => Promise<boolean> }) {
  ...
  return <section><SectionHeader title="Day-of check-in" copy="Choose a location and time, then tap each trained volunteer as they arrive." /><select value={shiftId} onChange={(event) => setShiftId(event.target.value)} className="mt-5 h-12 w-full max-w-2xl border-2 border-ink bg-paper-deep px-3"><option value="">Choose a shift…</option>{data.shifts.map((shift) => <option key={shift.id} value={shift.id}>{formatDay(shift.day)} · {formatTimeRange(shift.startsAt, shift.endsAt)} · {shift.location.shortName} · {shift.title ?? shift.task.name}</option>)}</select>{selected && <div className="mt-5"><div className="flex items-end justify-between border-b-2 border-ink pb-2"><h3 className="font-display text-2xl font-black">{selected.location.name}</h3><span className="eyebrow">{roster.filter((row) => row.status === 'checked_in').length} of {roster.length} here</span></div>{roster.length ? <div className="mt-3 grid gap-1 border-2 border-ink bg-ink sm:grid-cols-2">{roster.map((signup) => { const blocked = signup.status !== 'checked_in' && !signup.trainingComplete; return <button key={signup.id} onClick={() => toggle(signup)} disabled={pending === signup.id || blocked} className={`flex items-center gap-3 p-3 text-left disabled:cursor-not-allowed disabled:opacity-55 ${signup.status === 'checked_in' ? 'bg-success' : 'bg-paper'}`}><span className={`grid size-8 place-items-center border-2 border-ink ${signup.status === 'checked_in' ? 'bg-ink text-paper' : 'text-transparent'}`}><Check className="size-4" /></span><span><strong className="block">{signup.volunteerName}</strong><span className="text-sm text-ink-soft">{blocked ? 'Training required' : signup.phone}</span></span></button>; })}</div> : <Empty title="Nobody is signed up for this shift." copy="Choose another shift or return when registrations arrive." />}</div>}</section>;
}
