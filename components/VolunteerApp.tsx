          ...
          <section className="mx-auto max-w-6xl px-4 py-7 sm:px-7 sm:py-10">
            ...
            <div className="day-tabs mt-5" role="tablist" aria-label="Event days">
              {days.map((item) => {
                ...
                return <button key={item} type="button" role="tab" aria-selected={active} onClick={() => setDay(item)} className="day-tab"><span className="day-tab-label"><span className="block font-display text-base font-black sm:text-xl">{label.weekday}</span><span className="block text-[11px] font-bold uppercase tracking-[.08em]">{label.date}</span><span className="block text-[11px]">{open} open</span></span></button>;
              })}
            </div>
